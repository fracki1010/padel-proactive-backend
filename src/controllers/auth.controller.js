...
const updateMyCompany = async (req, res) => {
  try {
    ...
    if (hasCoverImage) company.coverImage = nextCoverImage;
    await company.save();

    const admin = await Admin.findById(req.user.id)
      .select("-password")
      .populate("companyId", "name slug address isActive coverImage");
...
const uploadCoverImage = async (req, res) => {
  try {
    ...
    const { uploadBuffer, configured } = require("../lib/cloudinary");
    if (!configured) {
